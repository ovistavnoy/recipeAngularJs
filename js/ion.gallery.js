  ...
  function ionGallery($ionicPlatform,ionGalleryData) {
    ...
    return {
      ...
      scope:{
        ...
        ionRemoveImage: '=ionRemoveImage',
        ionGalleryRow: '=ionGalleryRow'
        //ionGalleryImageRemove: '=ionGalleryImageRemove'
      },
    ...
    function controller($scope){
      ...
      ionGalleryData.setImageRemove($scope.ionGalleryImageRemove);
...
(function(){
  ...
  function ionGalleryData() {
    ...
    this.canRemove = function() {
      return imageRemove != undefined;
    };

    this.setImageRemove = function(method) {
      imageRemove = method;
    };

    this.doImageRemove = function(index) {
      imageRemove(index);
    };
...
(function(){
  ...
  function ionSlider($ionicModal,ionGalleryData,$ionicPlatform,$timeout,$ionicScrollDelegate){
    ...
    function controller($scope){
      ...
      $scope.hideAll = false;
      $scope.canRemove = ionGalleryData.canRemove();

      $scope.removeImage = function(index) {
        var nextSlide = index;
        if($scope.ionGalleryItems.length == 1) {
          $scope.closeModal();
        } else if($scope.ionGalleryItems.length-1 == index) {
          nextSlide = 0;
        }

        $scope.ionGalleryItems.splice(index, 1);

        $scope.showImage(0, nextSlide);
        //ionGalleryData.doImageRemove(index);

      };

      $scope.onHold = function(row,col) {
        $scope.ionRemoveImage(row*$scope.ionGalleryRow + col)
      };
